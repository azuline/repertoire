
# -*- coding: utf-8 -*-

# DO NOT EDIT THIS FILE!
# This file has been autogenerated by dephell <3
# https://github.com/dephell/dephell

try:
    from setuptools import setup
except ImportError:
    from distutils.core import setup

readme = ''

setup(
    long_description=readme,
    name='repertoire',
    version='0.1.0',
    description='A release-oriented music server.',
    python_requires='==3.*,>=3.9.0',
    author='blissful',
    author_email='blissful@sunsetglow.net',
    license='AGPL-3.0',
    entry_points={"console_scripts": ["repertoire = src.__main__:run"]},
    packages=['backend', 'backend.cli', 'backend.fixtures', 'backend.graphql', 'backend.graphql.types', 'backend.graphql.types.snapshots', 'backend.indexer', 'backend.indexer.snapshots', 'backend.library', 'backend.migrations', 'backend.search', 'backend.tasks', 'backend.webserver', 'backend.webserver.routes'],
    package_dir={"backend": "src"},
    package_data={"backend.graphql": ["*.gql"], "backend.migrations": ["sql/*.sql"]},
    install_requires=['ariadne==0.*,>=0.12.0', 'click==7.*,>=7.1.2', 'huey', 'pillow==8.*,>=8.1.0', 'python-dotenv==0.*,>=0.15.0', 'quart==0.*,>=0.14.1', 'requests==2.*,>=2.25.1', 'tagfiles==0.*,>=0.3.3', 'voluptuous==0.*,>=0.12.1', 'yoyo-migrations==7.*,>=7.3.1'],
    dependency_links=['git+https://github.com/azuline/huey.git@f46b1490e0854bc9c606a4ac023f651fd600a0dc#egg=huey'],
    extras_require={"dev": ["autodocsumm==0.*,>=0.2.2", "black==20.*,>=20.8.0.b1", "dephell==0.*,>=0.8.3", "filelock==3.*,>=3.0.12", "flake8==3.*,>=3.8.4", "freezegun==1.*,>=1.1.0", "isort==5.*,>=5.7.0", "mistune==0.*,>=0.8.4", "mypy==0.*,>=0.812.0", "pip-audit==1.*,>=1.1.0", "pytest==6.*,>=6.2.2", "pytest-asyncio==0.*,>=0.14.0", "pytest-cov==2.*,>=2.11.1", "pytest-xdist==2.*,>=2.2.1", "semgrep==0.*,>=0.53.0", "snapshottest==0.*,>=0.6.0", "sphinx==3.*,>=3.5.1", "sphinx-autodoc-typehints==1.*,>=1.11.1", "sphinx-rtd-theme==0.*,>=0.5.1", "sphinxcontrib-httpdomain==1.*,>=1.7.0"]},
)

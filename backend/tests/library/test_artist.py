...
def test_from_id_success(factory: Factory, db: Connection, snapshot):
...
def test_search_all(factory: Factory, db: Connection, snapshot):
...
def test_search_one(factory: Factory, db: Connection, snapshot):
    art = factory.artist(conn=db)
...
def test_search_page(factory: Factory, db: Connection, snapshot):
    arts = [factory.artist(conn=db) for _ in range(5)]
...
def test_search_per_page(factory: Factory, db: Connection, snapshot):
    arts = [factory.artist(conn=db) for _ in range(5)]
    arts = artist.search(db, page=1, per_page=2)
    assert len(arts) == 2


def test_count_all(factory: Factory, db: Connection, snapshot):
    arts = [factory.artist(conn=db) for _ in range(5)]
...
def test_count_search(factory: Factory, db: Connection, snapshot):
    arts = [factory.artist(conn=db) for _ in range(5)]
...
def test_update_fields(factory: Factory, db: Connection, snapshot):
    art = factory.artist(conn=db)
...
def test_update_duplicate(factory: Factory, db: Connection, snapshot):
...
def test_releases(factory: Factory, db: Connection, snapshot):
    art = factory.artist(conn=db)
    releases = {factory.release(artist_ids=[art.id], conn=db) for _ in range(4)}
    assert releases == set(artist.releases(art, db))


def test_top_genres(factory: Factory, db: Connection, snapshot):
    art = factory.artist(conn=db)
    releases = [factory.release(artist_ids=[art.id], conn=db) for _ in range(4)]
...
def test_image(factory: Factory, db: Connection):
    ...
    rls = factory.release(artist_ids=[art.id], image_id=img.id, conn=db)
